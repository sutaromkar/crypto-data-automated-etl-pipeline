...
def move_csv(bucket, key, file_names, index):
    # create S3 client
    s3 = boto3.client('s3')

    if (file_names != None):
        for f in file_names:
            s3.copy_object(Bucket='s3://crypto-etl-lake/archive_layer/', CopySource=bucket, Key=key + f)

    if (index != None):
        for i in range(1, index):
            s3.copy_object(Bucket='s3://crypto-etl-lake/archive_layer/', CopySource=bucket, Key=key + f'{i}.csv')
...
def transform_data(bucket):
    ...
    for f in file_names:
        ...
        df['PRICE'] = pd.to_numeric(df['PRICE'].str.replace('$', '').str.replace(',', '').str.replace(' ', ''),
                                    errors='coerce')
        df['PERCENT_CHANGE_24H'] = pd.to_numeric(
            df['PERCENT_CHANGE_24H'].str.replace('%', '').str.replace(',', '').str.replace(' ', ''), errors='coerce')
        df['VOLUME_24H'] = pd.to_numeric(
            df['VOLUME_24H'].str.replace('$', '').str.replace('B', 'E9').str.replace('M', 'E6').str.replace(',',
                                                                                                            '').str.replace(
                ' ', ''), errors='coerce')
        df['MARKET_CAP'] = pd.to_numeric(
            df['MARKET_CAP'].str.replace('$', '').str.replace('B', 'E9').str.replace('M', 'E6').str.replace(',',
                                                                                                            '').str.replace(
                ' ', ''), errors='coerce')
...
def load_data(bucket, conn, index):
    ...
    for i in range(1, index):
        ...
        querry = "copy {} FROM '{}' iam_role '{}'  CSV DELIMITER ',' IGNOREHEADER 1;".format("public.top_crypto_details",
                                                                                         from_path, iam_role)
        cur.execute(querry)
        conn.commit()
    print("load successfull")

    move_csv(bucket, 'transformation_layer/', None, index)
